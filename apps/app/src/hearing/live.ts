...
class LiveTranscribe {
  ...
  async startTranscription() {
    if (!this.mediaStream || !this.audioContext || !this.isServerReady || this.isPaused) return
  ...
  resetTranscription() {
    if (this.socket && this.isServerReady) {
      this.socket.send(JSON.stringify({ uid: this.uuid, status: 'RESET' }))
    }
  }
