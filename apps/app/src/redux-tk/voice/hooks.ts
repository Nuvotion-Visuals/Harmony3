...
export const useVoice_speaking = (): boolean => 
  useSelector(selectors.selectSpeaking, isEqual)
...
export const useVoice_setSpeaking = () => {
  const dispatch = useDispatch()
  return useCallback((speaking: boolean) => dispatch(voiceActions.setSpeaking(speaking)), [dispatch])
}
