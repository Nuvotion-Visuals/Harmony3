...
export const useVoice_playing = (): boolean => 
  useSelector(selectors.selectPlaying, isEqual)
...
export const useVoice_setPlaying = () => {
  const dispatch = useDispatch()
  return useCallback((playing: boolean) => dispatch(voiceActions.setPlaying(playing)), [dispatch])
}
